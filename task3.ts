...
function isAlarm(posA: Position, posB: Position): boolean {

  const mA: number = (posB.x - posA.x) * Math.cos(posA.angle) + (posB.y - posA.y) * Math.sin(posA.angle)
  const mB: number = (posA.x - posB.x) * Math.cos(posB.angle) + (posA.y - posB.y) * Math.sin(posB.angle)
...
const argsA: number[] = process.argv.slice(2, 5).map(parseFloat)
const argsB: number[] = process.argv.slice(5, 8).map(parseFloat)

const posA = new Position(argsA[0], argsA[1], argsA[2]);
const posB = new Position(argsB[0], argsB[1], argsB[2]);
